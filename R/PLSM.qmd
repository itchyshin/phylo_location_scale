...
Furthermore, we have another sister (you are in luck) paper, which teaches Bayesian generalised linear modelling both for continuous and discrete traits. You can find the tutorial [here](https://ayumi-495.github.io/multinomial-GLMM-tutorial/); also, the preprint of this sister paper is available on [EcoEvoRxiv](https://ecoevorxiv.org/repository/view/8458/):
...
form_loc <- bf(
  crange_size ~ 1 + cmass + 
    (1 | gr(Phylo, cov = A)) # this is how to fit phylogenetic effect
  # σ (residual SD) is left at its default single parameter
)

# (weakly-informative) priors
prior_loc <- default_prior(
  formula = form_loc,
  data    = dat,
  data2   = list(A = A),
  family  = gaussian()
...
saveRDS(mod_loc, here("Rdata", "mod_loc.rds"))
```
...
```{r}
#| warning: false
mod_loc <- readRDS(here("Rdata", "mod_loc.rds"))
...
plots_mod0 <- list(
  visualize_fixed_effects(mod_loc),
  visualize_random_effects(mod_loc)
)
